...
class supplierXpartRepository():
    ...
    def get_all_supplierXparts(self, branch:int) -> List[SupplierXpart]:
        query = self.db.query(supplierXpartModel).filter(supplierXpartModel.id_branch == branch)
        return query.all()
    
    def get_supplierXpart(self, id: int, branch:int) -> SupplierXpart:
        element = self.db.query(supplierXpartModel).filter(supplierXpartModel.id == id, supplierXpartModel.id_branch == branch).first()
        return element
    ...
    def remove_supplierXpart(self, id: int, branch:int ) -> dict:
        element: SupplierXpart = self.db.query(supplierXpartModel).filter(supplierXpartModel.id == id, supplierXpartModel.id_branch == branch).first()
